...
if option == 'Sondagens e Resultados':
    ...
    st.write(polls.style.format({'PS': '{:0,.1f}','AD': '{:0,.1f}',
   #'CDS': '{:0,.1f}',
    'BE': '{:0,.1f}', 'CDU': '{:0,.1f}','PAN': '{:0,.1f}',
    ...
    pp=detailed_district.groupby('circulo').agg(f).astype(int)
...
if option == 'Partidos e Coligações':
   ...
   #     string_col = string_col + ' CDS-PP '
    ...
    num_partidos = ps+ad+cdu+be+ch+il+pan+livre
...
if option == 'Circulos Eleitorais':
    ...
    seats = {'Acores':5, 'Aveiro':16, 'Beja':3, 'Braga':19, 'Braganca':3, 'Castelo Branco':4,
       ...
       'Portalegre':2, 'Porto':40, 'Santarem':9, 'Setubal':19, 'Viana do Castelo':5,
...
if option == 'Calculadora da Viabilidade':
  ...
  #   options=['viabiliza', 'abstenção', 'rejeita'], key=2)
    be_v = col3.select_slider(
     'Opção de voto do BE',
     options=['viabiliza', 'abstenção', 'rejeita'], key=2)
    cdu_v = col1.select_slider(
     'Opção de voto da CDU',
     options=['viabiliza', 'abstenção', 'rejeita'], key=3)
    il_v = col3.select_slider(
     'Opção de voto da IL',
     options=['viabiliza', 'abstenção', 'rejeita'], key=4)
    ch_v = col1.select_slider(
     'Opção de voto do Chega',
     options=['viabiliza', 'abstenção', 'rejeita'], key=5)
    pan_v = col3.select_slider(
     'Opção de voto do PAN',
     options=['viabiliza', 'abstenção', 'rejeita'], key=6)
    livre_v = col1.select_slider(
     'Opção de voto do LIVRE',
     options=['viabiliza', 'abstenção', 'rejeita'], key=7)
    ...
    if formador == 'PS':
        if ad_v == 'viabiliza':
            lista_favor.append('AD')
        elif ad_v == 'rejeita':
            lista_contra.append('AD')
        else:
            lista_abstencao.append('AD')
    else:
        if ps_v == 'viabiliza':
...
if option == 'Quadro-Resumo':
    ...
    c1,c2,c3 = st.columns(3)
    c1.metric("PS", str(round(sim_df.query('PS > AD').shape[0]/100,1))+'%',"")
    c2.metric("AD", str(round(sim_df.query('PS < AD').shape[0]/100,1))+'%',"")
    c3.metric("Empate", str(round(sim_df.query('PS == AD').shape[0]/100,1))+'%',"")
